...
submit() { local arm=$1; local dep=${2:-}; local depflag=(); [[ -n "$dep" ]] && depflag=(--dependency="afterok:$dep"); sbatch --parsable "${depflag[@]}" --job-name="qcpr-${arm}-matched" --account=research --partition=research --qos=researcher --gres=gpu:1 --cpus-per-task=16 --mem=100G --time=08:00:00 --output="$OUTPUT_ROOT/${arm}-%j.out" --wrap="set -eu; cd '$ROOT'; test \"\$(git rev-parse HEAD)\" = '$EXPECTED_SHA'; test -z \"\$(git status --porcelain)\"; test -s '$BASELINE'; test -s '$REPRO'; test \"\$(jq -r '.passed' '$REPRO')\" = true; test \"\$(sha256sum '$BASELINE' | cut -d ' ' -f1)\" = \"\$(jq -r '.checkpoint_sha256' '$REPRO')\"; export PYTHONPATH='$ROOT/src:$ROOT/scripts'\${PYTHONPATH:+:\$PYTHONPATH}; exec '$PYTHON' scripts/train_qcpr_p0_p1_control.py --arm '$arm' --train-manifest '$CONTROL_ROOT/${arm}_train.jsonl' --development-manifest '$CONTROL_ROOT/${arm}_development.jsonl' --relevance-manifest '$CONTROL_ROOT/${arm}_relevance.jsonl' --output-dir '$OUTPUT_ROOT/$arm' ${COMMON[*]}"
}
