...
J2=$(sbatch --parsable --partition=research --dependency=afterok:$J1 --job-name=qcpr-query-grounding-full --gres=gpu:1 --cpus-per-task=12 --mem=140G --time=48:00:00 --output="$RUN_ROOT/logs/grounding-%j.out" --wrap="$COMMON; test -f $RUN_ROOT/retrieval/best_retrieval.pt; $PYTHON scripts/train_qcpr_query_localization.py --retrieval-checkpoint $RUN_ROOT/retrieval/best_retrieval.pt --output-dir $RUN_ROOT/grounding --manifest-dir $MANIFEST_DIR --output-grid 32 --query-batch 8 --candidates 4")
