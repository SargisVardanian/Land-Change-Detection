...
JOB_ID="$(sbatch --parsable \
  ...
  --wrap="set -eu; cd \"$WORKTREE\"; test \"\$(git rev-parse HEAD)\" = \"$EXPECTED_SHA\"; test -z \"\$(git status --porcelain)\"; test -f \"$DATA_RELEASE/SHA256SUMS\"; test -f \"$CHECKPOINT_PATH\"; export PYTHONPATH=src:scripts; exec \"$PYTHON_BIN\" scripts/compare_qcpr_siglip2_resolution_compat.py --naflex-model \"$NAFLEX_MODEL\" --fixres-model \"$FIXRES_MODEL\" --data-release \"$DATA_RELEASE\" --development-manifest \"$DEVELOPMENT_MANIFEST\" --config-path \"$CONFIG_PATH\" --checkpoint \"$CHECKPOINT_PATH\" --output-dir \"$RUN_ROOT\" --expected-code-sha \"$EXPECTED_SHA\" --max-pairs \"${MAX_PAIRS:-32}\"")"
printf '%s\n' "$JOB_ID"
