...
: "${NAFLEX_REPOSITORY:?NAFLEX_REPOSITORY is required}"
: "${FIXRES_REPOSITORY:?FIXRES_REPOSITORY is required}"
: "${NAFLEX_REVISION:?NAFLEX_REVISION is required}"
: "${FIXRES_REVISION:?FIXRES_REVISION is required}"
...
JOB_ID="$(sbatch --parsable \
  ...
  --wrap="set -eu; cd \"$WORKTREE\"; test \"\$(git rev-parse HEAD)\" = \"$EXPECTED_SHA\"; test -z \"\$(git status --porcelain)\"; test -f \"$DATA_RELEASE/SHA256SUMS\"; test -f \"$CHECKPOINT_PATH\"; export PYTHONPATH=src:scripts; exec \"$PYTHON_BIN\" scripts/compare_qcpr_siglip2_resolution_compat.py --naflex-model \"$NAFLEX_MODEL\" --fixres-model \"$FIXRES_MODEL\" --naflex-repository \"$NAFLEX_REPOSITORY\" --fixres-repository \"$FIXRES_REPOSITORY\" --naflex-revision \"$NAFLEX_REVISION\" --fixres-revision \"$FIXRES_REVISION\" --data-release \"$DATA_RELEASE\" --development-manifest \"$DEVELOPMENT_MANIFEST\" --config-path \"$CONFIG_PATH\" --checkpoint \"$CHECKPOINT_PATH\" --output-dir \"$RUN_ROOT\" --expected-code-sha \"$EXPECTED_SHA\" --max-pairs \"${MAX_PAIRS:-0}\" --image-batch-size \"${IMAGE_BATCH_SIZE:-8}\" --text-batch-size \"${TEXT_BATCH_SIZE:-64}\" --bootstrap-replicates \"${BOOTSTRAP_REPLICATES:-1000}\"")"
printf '%s\n' "$JOB_ID"
