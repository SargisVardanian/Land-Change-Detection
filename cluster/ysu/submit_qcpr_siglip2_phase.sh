...
MAX_UNIQUE_CAPTIONS_PER_PAIR=${MAX_UNIQUE_CAPTIONS_PER_PAIR:-2}
SEED=${SEED:-20260805}
...
export STEPS PHYSICAL_BATCH_SIZE LOGICAL_PHYSICAL_BATCH_SIZE MAX_UNIQUE_CAPTIONS_PER_PAIR SEED
...
exec \"\$PYTHON\" \"\$WORKTREE/scripts/run_qcpr_siglip2_phase.py\" \\
  ...
  --max-unique-captions-per-pair \"\$MAX_UNIQUE_CAPTIONS_PER_PAIR\" \\
  --seed \"\$SEED\" \\
