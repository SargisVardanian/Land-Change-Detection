...
sbatch --parsable \
  ...
  --wrap="set -eu; cd \"$WORKTREE\"; test \"\$(git rev-parse HEAD)\" = \"$EXPECTED_SHA\"; test -z \"\$(git status --porcelain)\"; export PYTHONPATH=src:scripts; exec \"$PYTHON_BIN\" scripts/collect_qcpr_slurm_completion.py --job-id \"$UPSTREAM_JOB_ID\" --collector-job-id \"\$SLURM_JOB_ID\" --run-root \"$RUN_ROOT\" --expected-code-sha \"$EXPECTED_SHA\""
