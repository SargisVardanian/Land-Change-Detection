...
class LayerScaleTransformerBlock(nn.Module):
    ...
    def forward(self, hidden: Tensor) -> Tensor:
        ...
        hidden = hidden + attention * self.attn_scale
        return hidden + self.ffn(self.norm_ffn(hidden)) * self.ffn_scale
...
class TemporalTransformerAdapter(nn.Module):
    ...
    def _spatial_position(
        ...
        side = math.isqrt(n)
        if side * side != n:
            raise ValueError("native patch count must form a square grid")
        value = F.interpolate(
        ...
        return value.flatten(2).transpose(1, 2).to(device=device, dtype=dtype)
    ...
    def forward(
        ...
        spatial = self._spatial_position(
            n, device=frame_tokens.device, dtype=frame_tokens.dtype
        )
        if timestamps is None:
            tf = torch.zeros(
                (b, t, 2), device=frame_tokens.device, dtype=frame_tokens.dtype
            )
        else:
            ...
            tf = torch.stack((timestamps - first, delta / scale), dim=-1).to(
                frame_tokens.dtype
            )
        ...
        time_bias = self.time_projection(
            tf.to(dtype=self.time_projection.weight.dtype)
        ).to(dtype=frame_tokens.dtype)
        seq = [pair_initial.unsqueeze(1)]
        for i in range(t):
