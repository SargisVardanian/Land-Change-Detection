...
def _interpret_feature_shift(cosine_distance: float, l2_distance: float) -> str:
    if cosine_distance >= 0.22:
        return "strong feature shift; inspect for meaningful surface or layout change"
    if cosine_distance >= 0.14:
        return "moderate feature shift; possible construction, grading, road change, or layout rework"
    if cosine_distance >= 0.08:
        return "weak to moderate feature shift; possible local surface disturbance or texture change"
    return "low feature shift; mostly stable visual features"


class DINOv3FeatureEncoder:
    """DINOv3 SAT feature extractor for debug diagnostics only.

    This class intentionally does not expose semantic segmentation. DINOv3 is a
    backbone here; named land-cover segmentation requires a trained decoder/head.
    """

    def __init__(self, model_name_or_path: str = DINO_V3_VITL16_SAT, device: str = "cpu"):
        self.model_name_or_path = model_name_or_path
        self.device = torch.device(device)
        self.dtype = torch.float16 if self.device.type in {"mps", "cuda"} else torch.float32
        repo_id = model_name_or_path[len("hf_hub:") :] if model_name_or_path.startswith("hf_hub:") else model_name_or_path
    ...
    def analyze_grid(self, before_rgb: np.ndarray, after_rgb: np.ndarray, grid_size: int = 4) -> list[DINOCellShift]:
        ...
        for row_idx in range(grid_size):
            for col_idx in range(grid_size):
                ...
                emb_before = self.encode(before_arr[top:bottom, left:right])
                emb_after = self.encode(after_arr[top:bottom, left:right])
