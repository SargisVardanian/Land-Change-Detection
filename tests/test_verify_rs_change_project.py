...
def test_build_report_ready(tmp_path: Path):
    ...
    assert report["levir_cc_required_complete"] is True
    ...
    assert report["levir_cc_runs"]["simple_patch_smoke"]["overfit_gate_passed"] is True
    assert report["levir_cc_runs"]["simple_patch_smoke"]["overfit_artifacts_complete"] is False
    assert report["levir_cc_runs"]["simple_patch_smoke"]["train_artifacts_complete"] is False
...
def test_build_report_marks_levir_cc_milestone_ready_when_artifacts_and_gate_exist(tmp_path: Path):
    ...
    for directory in [overfit_dir, train_dir]:
    ...
    for rel, payload in [
        ...
        (train_dir / "train_summary.json", '{"best_epoch": 4}\n'),
    ...
    report = module.build_report(project_root)
    levir_report = report["levir_cc_runs"]["simple_patch_smoke"]
    assert levir_report["overfit_gate_passed"] is True
    assert levir_report["overfit_artifacts_complete"] is True
    assert levir_report["train_artifacts_complete"] is True
    assert levir_report["milestone_ready"] is True
    assert levir_report["train_best_epoch"] == 4
    assert report["levir_cc_any_milestone_ready"] is True
