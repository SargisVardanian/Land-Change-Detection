...
function render(){const row=current(),saved=answers[row.review_id]||blank();document.getElementById('progress').textContent=`${index+1} / ${ROWS.length}`;document.getElementById('role-help').textContent=ROLE_HELP;document.getElementById('content').innerHTML=`<section class="panel"><div class="pair-meta"><div><span class="meta-label">Review ID</span><span class="meta-value">${row.review_id}</span></div><div><span class="meta-label">Pair</span><span class="meta-value">${row.canonical_pair_id}</span></div><div><span class="meta-label">Event</span><span class="meta-value">${row.source_event_id}</span></div><div><span class="meta-label">Split</span><span class="meta-value">${row.split}</span></div></div><div class="images"><figure class="image-card"><img src="${row.t1_rel}" alt="T1"><figcaption>T1 · первая дата</figcaption></figure><figure class="image-card"><img src="${row.t2_rel}" alt="T2"><figcaption>T2 · вторая дата</figcaption></figure></div><div class="caption-box"><div class="caption-title">Кандидатное описание · не доверяйте ему автоматически</div><div class="caption-text">${escapeHtml(row.candidate_caption)}</div></div><div class="role-note">${ROLE_HELP}</div><div class="fields">${renderFields()}</div></section>`;document.querySelectorAll('[data-field]').forEach(el=>el.value=saved[el.dataset.field]||'')}
