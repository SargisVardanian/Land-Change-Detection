...
def build_report(host: str, ssh_alias: str, ssh_config: Path, wireguard_dir: Path, timeout: int) -> dict:
    resolved = _resolve_host(host)
    diagnostics = {
        "host": host,
        "resolved_addresses": resolved,
        "ssh_config_exists": ssh_config.exists(),
        "ssh_alias_present": _ssh_alias_present(ssh_config, ssh_alias),
    ...
    ssh_target = ssh_alias if diagnostics["ssh_alias_present"] else host
    ...
    diagnostics["ready_for_bootstrap"] = bool(resolved) and bool(https_ok) and bool(ssh_ok)
    return diagnostics


def main() -> int:
    args = parse_args()
    report = build_report(args.host, args.ssh_alias, args.ssh_config, args.wireguard_dir, args.timeout)
