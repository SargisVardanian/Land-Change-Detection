...
def main() -> None:
    ...
    (args.run_root / "model_contract.json").write_text(json.dumps({"architecture_id": config.architecture_id, "sequence_cls": [pair_count, 512], "dense_tokens": [pair_count, time_count, token_count, 512], "evidence_map": [query_count, pair_count, time_count, token_count], "parameter_counts": model.parameter_counts(), "parameter_groups": model.parameter_group_counts()}, indent=2) + "\n")
    (args.run_root / "parameter_groups.json").write_text(json.dumps(model.parameter_group_counts(), indent=2, sort_keys=True) + "\n")
