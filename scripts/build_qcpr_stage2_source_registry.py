...
def source(name: str, official: list[str], raw: Path, roles: list[str], license_name: str, state: str, *, blocker: str|None=None, version: str|None=None, pair_count: int|None=None, caption_count: int|None=None) -> dict[str,Any]:
    if state not in STATES: raise ValueError(state)
    return {"source_dataset":name,"official_locations":official,"version_or_revision":version,"license":license_name,"raw_inventory":inventory(raw),"state":state,"roles":roles,"physical_pair_count":pair_count,"text_count":caption_count,"dense_label_count":None,"official_split":None,"sensor":None,"native_dimensions":None,"gsd":None,"temporal_metadata":None,"accessibility":"blocked" if blocker else "available","blocker":blocker,"loader_validation":{"passed":False,"artifact":None}}
...
def main() -> int:
    ...
    rcd_ok=all(x.is_file() and x.stat().st_size>0 for x in required); ebd_ok=all((ebd/f"EBD.tar.gz-part-{i}").is_file() and (ebd/f"EBD.tar.gz-part-{i}").stat().st_size>0 for i in range(5))
    rows=[
