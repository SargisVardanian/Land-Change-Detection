...
def score(model, corpus, device, qchunk, cchunk, include_masks):
    ...
    local, token_patch, reranked, logits, targets = torch.empty_like(global_scores), torch.empty_like(global_scores), torch.empty_like(global_scores), [], []
    for q0 in range(0, qcount, qchunk):
        ...
        for c0 in range(0, ccount, cchunk):
            ...
            output = model.score_encoded(corpus["pairs"][c0:c1].to(device), corpus["per_time"][c0:c1].to(device), corpus["text"][q0:q1].to(device), corpus["tokens"][q0:q1].to(device), corpus["attention"][q0:q1].to(device), corpus["content"][q0:q1].to(device))
            local[q0:q1, c0:c1] = output.local_score.cpu()
            token_patch[q0:q1, c0:c1] = output.token_patch_score.cpu()
            reranked[q0:q1, c0:c1] = output.reranked_score.cpu()
            if include_masks:
...
def main() -> int:
    ...
    global_scores, local, token_patch, reranked, logits, targets = score(model, corpus, device, args.query_chunk, args.candidate_chunk, args.phase == "mask_grounding")
