...
def main() -> int:
    ...
    structured_independent_ready=(structured_independent.get("status")=="INDEPENDENT_STRUCTURED_SEMANTIC_VERIFIED" and bool(structured_independent.get("mask_free_output")) and int(structured_independent.get("verified_rows",0))>0)
    structured_ready=structured.get("status")=="STRUCTURED_SOURCE_SEMANTIC_READY" and bool(structured.get("training_enabled")) and bool(structured_loader.get("passed")) and structured_independent_ready
    ...
    payload={
        ...
        "semantic":{"compact_group_registry":semantic.get("compact_group_registry",False),"counts":semantic.get("counts"),"groups":semantic.get("groups"),"multi_positive_rows":semantic.get("multi_positive_rows"),"single_positive_rows":semantic.get("single_positive_rows"),"materialized_positive_rows":semantic.get("materialized_positive_rows"),"training_enabled_rows":semantic.get("training_enabled_rows"),"structured_source_rows":semantic.get("structured_source_rows"),"human_audit_rows":semantic.get("human_audit_rows"),"manual_visual_verified_rows":manual.get("verified_semantic_rows",0),"manual_visual_status":manual.get("status"),"review_required_rows":semantic.get("review_required_rows"),"automated_verified_rows":automated.get("row_count",0),"automated_verified_split_counts":automated.get("split_counts",{}),"automated_verification_status":automated.get("status"),"status":semantic.get("status"),"gate":semantic.get("stage2_gate"),"train_sha256":sha256(audit/"semantic_view/retrieval_semantic_train_v2.jsonl"),"development_sha256":sha256(audit/"semantic_view/retrieval_semantic_development_v2.jsonl"),"test_sha256":sha256(audit/"semantic_view/retrieval_semantic_test_v2.jsonl"),"semantic_group_registry_sha256":sha256(audit/"semantic_view/semantic_group_registry.jsonl"),"structured_source_verified":{"status":structured.get("status"),"rows":structured.get("output_row_count",0),"split_counts":structured.get("output_split_counts",{}),"groups":structured.get("group_count",0),"training_enabled":structured.get("training_enabled",False),"human_verified":structured.get("human_verified",False),"independent_verified":structured_independent_ready,"independent_rows":structured_independent.get("verified_rows",0),"audit_path":str(structured_path),"audit_sha256":sha256(structured_path),"loader_passed":structured_loader.get("passed",False),"loader_path":str(structured_loader_path),"loader_sha256":sha256(structured_loader_path),"independent_path":str(structured_independent_path),"independent_sha256":sha256(structured_independent_path)}},
        "structured_semantic_view_ready":structured_ready,
